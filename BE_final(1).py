...
def Visu(Coefs, nfenetres):
    """Affiche le spectre du filtre LPC en dB."""
    p = Coefs.shape[1] - 1  # Nombre de coefficients LPC
    for i in range(nfenetres):
        w, h = sps.freqz(1, Coefs[i, :], worN=512)
        plt.plot(w, 20 * np.log10(abs(h)), label=f'Trame {i}')
...
def test_speech_lpc():
    ...
    # Sauvegarde
    sf.write("speech_reconstructed.wav", x_reconstructed, 8000)
...
def reconstruct_speech_with_lpc():
    ...
    x_reconstructed = np.clip(x_reconstructed, -1.0, 1.0)
    x_reconstructed = x_reconstructed / np.max(np.abs(x_reconstructed)) * 0.9
    sf.write("speechLPC.wav", x_reconstructed, 8000)
